...
import com.bridge.enums.BridgeEnableEnum;
...
public class BridgePropertySourceBeanDefinitionParser extends AbstractBeanDefinitionParser {
    ...
    protected AbstractBeanDefinition parseInternal(Element element, ParserContext parserContext) {
        ...
        BridgeEnableEnum enable = BridgeEnableEnum.isContains(element.getAttribute(ENABLE));
        properties.setProperty(ENABLE, enable == null ? BridgeEnableEnum.TRUE.toString() : enable.toString());
        // 处理占位符的问题，如果存在的话
        Properties bridgeConfig = PropUtils.resolveByPropertyResolver(properties, environment);
        // 没有开启配置中心则停止初始化
        String enableProperties = bridgeConfig.getProperty(ENABLE);
        if (enableProperties != null && !enableProperties.equalsIgnoreCase(Boolean.TRUE.toString())) {
            return null;
        }
